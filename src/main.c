...
void resolveSL(pLU ls){
    for (int i = 0; i < ls->order; i++) {
        ...
        for (int j = 0; j < i; j++)
            ls->Y[ls->access_mask[i]] -= access(ls->L, i, j)*ls->Y[ls->access_mask[j]];
    }
    
    for (int i = ls->order-1; i >= 0; --i) {
        ls->X[ls->access_mask[i]] = ls->Y[ls->access_mask[i]];
        for (int j = i+1; j < ls->order; j++)
            ls->X[ls->access_mask[i]] -= access(ls->U, i, j) * ls->X[ls->access_mask[j]];
            ls->X[ls->access_mask[i]] /= access(ls->U, i, i);
    }
}
